...
bool Boid::act(glm::vec3 goal_pos, std::vector<Obstacle*> obstacles, glm::vec3 flock_center, std::vector<Boid>& boids) {
    ...
    std::vector<glm::vec3> path = aStar(position, goal_pos, obstacles);
    std::cout << "DEBUG: " << path.size() << "\n";
...
float Boid::euclideanDistance(const glm::vec3& a, const glm::vec3& b)
{
    return std::sqrt((std::pow(a.x -b.x,2)) + (std::pow(a.y-b.y,2)) + (std::pow(a.z-b.z,2)));
}

//Chebyshev Distance heuristic
float Boid::chebyshevDistance(const glm::vec3& a, const glm::vec3& b)
{
...
float Boid::manhattanDistance(const glm::vec3& a, const glm::vec3& b) {
    return std::abs(a.x - b.x) + std::abs(a.y - b.y) + std::abs(a.z - b.z);
}

std::vector<glm::vec3> Boid::aStar(glm::vec3 start, glm::vec3 goal, std::vector<Obstacle*> boxes) {
...
void Boid::drawPath(const std::vector<glm::vec3>& path, glm::vec3 boid_pos) {
    glLineWidth(5.0f);
